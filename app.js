...
const firebaseConfig = {
 ...
 authDomain: "custbook-eea44.firebaseapp.com",
 projectId: "custbook-eea44",
 storageBucket: "custbook-eea44.firebasestorage.app",
...
(function () {
 ...
 // ===== STORAGE KEYS =====
 const STORAGE_KEY = 'custbook_data';
 const SETTINGS_KEY = 'custbook_settings';
 const AUTH_KEY = 'custbook_auth';
 ...
 const count = parseInt(localStorage.getItem('custbook_receipt_count') || '0') + 1;
 localStorage.setItem('custbook_receipt_count', count.toString());
 ...
 try {
 const users = JSON.parse(localStorage.getItem('custbook_users') || '[]');
 ...
 localStorage.setItem('custbook_users', JSON.stringify(users));
 } catch { /* ignore */ }
 }
 ...
 showMainApp();
 showToast(`Welcome to CustBook, ${name.split(' ')[0]}!`);
 }
 ...
 <button class="txn-action-btn delete" data-txn-id="${txn.id}" title="Delete">Delete</button>
 </div>
 `;
 ...
 a.href = url;
 a.download = `custbook-backup-${new Date().toISOString().split('T')[0]}.json`;
 a.click();
